...
class Settings:
    ...
    @staticmethod
    def get(key, default=None, key_type=str):
        ...
        if _settings.contains(key):
            return _settings.value(key, type=key_type)
    ...
    @staticmethod
    def persisted_property(key, default_value=None, persist_if=lambda self: True, key_type=str):
        """
        ...
        :param key_type: Type of values for persisting
        :return: a property suitable for a class
        """
        return property(lambda s: Settings.get(key, default=default_value, key_type=key_type),
...
def make_dirs():
    ...
    for game_dir in [
        ...
        path = Settings.get(game_dir)
        if path is None:
            raise Exception("Missing configured path for {}".format(game_dir))
...
VERSION = version.get_release_version(dir=fafpath.get_resdir(), git_dir=fafpath.get_srcdir()).replace('\n', '')
